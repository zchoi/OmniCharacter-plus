...
class RobActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        special_tokens = [f'<action_{i}>' for i in range(256)]
        existing_tokens = set(self.processor.tokenizer.additional_special_tokens)
        ...
        actor_module = Qwen3VLForConditionalGeneration.from_pretrained(
            model_path, dtype=torch.bfloat16, local_files_only=True
        )

        actor_module_config = actor_module.config
        ...
        # TODO(zhangchi.usc1992): 1. support create from random initialized model. 2. Support init with FSDP directl
        ...
        # use_orig_params=True so that value_head.parameters() is non-empty after wrap (FSDP with
        # use_orig_params=False flattens params and submodule.parameters() can be empty).
        ...
        if self._is_actor:
            ...
            value_head_params = list(actor_module_fsdp.value_head.parameters())
            value_head_param_ids = {id(p) for p in value_head_params}
            actor_params = [p for p in actor_module_fsdp.parameters() if id(p) not in value_head_param_ids]
            actor_optimizer = optim.AdamW(actor_params,
                                          lr=optim_config.lr,
                                          betas=optim_config.get('betas', (0.9, 0.999)),
                                          weight_decay=optim_config.get('weight_decay', 1e-2))

            if len(value_head_params) == 0:
                raise ValueError(
                    "value_head.parameters() is empty. Check that the model has a value_head submodule with parameters "
                    "and that FSDP is created with use_orig_params=True."
                )
            value_head_optimizer = optim.AdamW(value_head_params,
                                          lr=optim_config.value_head_lr,
                                          betas=optim_config.get('betas', (0.9, 0.999)),
                                          weight_decay=optim_config.get('weight_decay', 1e-2))
            ...
            value_head_lr_scheduler = get_constant_schedule_with_warmup(optimizer=value_head_optimizer,
                                                                       num_warmup_steps=num_warmup_steps)
        else:
            actor_optimizer = None
            actor_lr_scheduler = None
            value_head_optimizer = None
            value_head_lr_scheduler = None

        log_gpu_memory_usage('After actor optimizer init', logger=logger)

        return actor_module_fsdp, actor_optimizer, actor_lr_scheduler, value_head_optimizer, value_head_lr_scheduler, actor_module_config
    ...
    def init_model(self):
        ...
        if self._is_actor or self._is_rollout:
            ...
            self.actor_module_fsdp, self.actor_optimizer, self.actor_lr_scheduler, self.value_head_optimizer, self.value_head_lr_scheduler, self.actor_model_config = self._build_model_optimizer(
            ...
            if self._is_offload_optimizer:
                ...
                offload_fsdp_optimizer(optimizer=self.value_head_optimizer)
        ...
        if self._is_actor:
            ...
            self.actor = RobDataParallelPPOActor(config=self.config.actor,
                                              ...
                                              value_head_optimizer=self.value_head_optimizer)
    ...
    def update_actor(self, data: DataProto):
        ...
        if self._is_offload_optimizer:
            ...
            load_fsdp_optimizer(optimizer=self.value_head_optimizer, device_id=torch.cuda.current_device())
        ...
        self.value_head_lr_scheduler.step()
        lr = self.actor_lr_scheduler.get_last_lr()[0]
        value_head_lr = self.value_head_lr_scheduler.get_last_lr()[0]
        metrics['actor/lr(1e-4)'] = lr * 1e4
        metrics['value_head/lr(1e-4)'] = value_head_lr * 1e4
    ...
    def set_lr_scale(self, scale: float):
        """Multiply the learning rate of actor and value head optimizers by `scale`."""
        assert self._is_actor
        for optimizer, scheduler in [
            (self.actor_optimizer, self.actor_lr_scheduler),
            (self.value_head_optimizer, self.value_head_lr_scheduler),
        ]:
            for param_group in optimizer.param_groups:
                param_group['lr'] *= scale
                if 'initial_lr' in param_group:
                    param_group['initial_lr'] *= scale
            scheduler.base_lrs = [lr * scale for lr in scheduler.base_lrs]
    ...
    def generate_sequences(self, prompts):
        ...
        if self._is_actor and recompute_log_prob:
            ...
            # traj_len = output.batch['responses'].size(1)
            # old_log_probs = output.batch['old_log_probs'].reshape(output.batch['old_log_probs'].shape[0], traj_len)
    ...
    def save_checkpoint(self, local_path, hdfs_path=None):
        ...
        #lora add: only import peft when using LoRA to avoid accelerate version conflict
        do_lora_save = False
        if self._is_lora:
            from peft import PeftModel
            do_lora_save = isinstance(self.actor_module, PeftModel)
        if do_lora_save:
            if dist.get_rank() == 0:
        ...
        # TODO: support DCP and save sharded checkpoints
            ...
            if self.rank == 0:
                ...
                with open(os.path.join(local_path, 'statistics.json'), 'w') as f:
                    json.dump(self.rollout.stats_data, f, indent=2)
