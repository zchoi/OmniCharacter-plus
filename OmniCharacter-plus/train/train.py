...
def preprocess(
    ...
    if conversation_lib.default_conversation.version == "qwen2":
        # print('--qwen_v2--')
        return preprocess_qwen_2(sources, tokenizer, tune_speech_generator, sys_prompt)
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'speech' in sources[0]:
            assert len(sources[0]["conversations"]) == len(sources[0]["speech"])
            for idx, (a, b) in enumerate(zip([i for i in sources[0]["conversations"]], sources[0]["speech"])):
                if random.random() < self.speech_prob and 'none' not in b:  
                    temp_path = os.path.join(speech_folder, b)
                    if not os.path.exists(temp_path):
                        print(f"the audio file does not exists!!!, {temp_path}")
                        continue
